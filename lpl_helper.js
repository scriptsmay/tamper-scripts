// ==UserScript==
// @name         LPL赛事数据助手
// @namespace    lpl_helper
// @version      0.0.1
// @description  1. 没做完，先搁置
// @author       scriptsmay
// @match        *://lpl.qq.com/*
...
// @downloadURL  https://raw.githubusercontent.com/scriptsmay/tamper-scripts/main/lpl_helper.js
// @updateURL    https://raw.githubusercontent.com/scriptsmay/tamper-scripts/main/lpl_helper.js
// ==/UserScript==
...
(function () {
  ...
  const CONFIG = {
    ...
    hideStyle: 'JS_LPLHELPER_hide',
    bodyStyle: 'JS_LPLHELPER_doc',
    style0: 'JS_LPLHELPER_pd0',
  ...
  const mainSetStyle = `
    ...
    .JS_LPLHELPER_hide { display: none!important;}
    .JS_LPLHELPER_pd0 {
        padding: 0!important;
    }
    .data-main-b { height: auto!important; }  
    .data-main-b img, .data-main-b label { display: block; }
    .data-main.fl .data-main-b img { float: none;}
    .data-main.fr .data-main-b img { float: none;}
  `;
  ...
  class LPLPageClass extends BaseClass {
    ...
    async appendText() {
      ...
      const heroImgs = await BaseClass.getElement('.data-main-b img', true);
      if (heroImgs && heroImgs.length > 0) {
        heroImgs.forEach((item) => {
          const text = item.getAttribute('title');
          // 创建 label 元素
          const label = document.createElement('label');
          label.textContent = text; // 设置 label 的文本内容为 title 属性值

          // 将 label 插入到 item 元素之后
          item.parentNode.insertBefore(label, item.nextSibling);
        });
      }
    }
