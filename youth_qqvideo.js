// ==UserScript==
// @name         腾讯视频少儿模式、芒果tv去广告
// @namespace    youth-qqvideo
// @version      0.0.5
// @description  1. 腾讯视频网页默认进入少儿模式，导航只显示动漫和少儿，尝试屏蔽部分广告区域。2. 芒果TV屏蔽广告区域。updateTime:2024-05-11 19:44
// @author       scriptsmay
// @match        *://*.youku.com/*
// @match        *://*.iqiyi.com/*
// @match        *://v.qq.com/*
...
(function () {
    ...
    class PlayVideoClass extends BaseClass {
        ...
        async playPage() {
            ...
            const mgPlay = await BaseClass.getElement('.g-play');
            if (!isPlayPage && !mgPlay) {
                console.log('非播放页');
                return false;
            }
            if (isPlayPage) {
            ...
            if (mgPlay) {
                // 关闭芒果TV弹幕
                document.getElementsByClassName('danmu_toolbar')[0].firstChild.click();
            }
