...
(function () {
      ...
      GM_cookie.list({ url: '.weibo.com' }, function (cookies, error) {
        if (error) {
          ...
          GM_xmlhttpRequest({
            ...
            onerror: function (e) {
              downloadError(e, url, fileName, headerFlag, progress);
              resolve(null);
            },
        ...
        const download = GM_xmlhttpRequest({
          ...
          onprogress: (e) => {
            ...
            progress.style.background =
              'linear-gradient(to right, green ' + percent + '%, transparent ' + percent + '%)';
          ...
          onerror: function (e) {
            downloadError(e, url, name, headerFlag, progress);
            resolve(null);
          },
          ontimeout: function (e) {
            downloadError(e, url, name, headerFlag, progress);
    ...
    if (GM_getValue('ariaMode', false)) {
      ...
      Promise.all(promises).then(async function (responseList) {
        ...
        const content = await zip.generateAsync(
          { type: 'blob', streamFiles: true } /*, function({ percent, currentFile }) { console.log(percent); }*/
        );
  ...
  async function handleVideo(
    ...
    const newList = [];
    let largeVidUrl = mediaInfo.playback_list
      ? mediaInfo.playback_list[0].play_info.url
      : mediaInfo.mp4_hd_url || mediaInfo.stream_url_hd || mediaInfo.stream_url;
    if (mediaInfo.hasOwnProperty('h5_url') && mediaInfo.h5_url) {
      ...
      if (
        tvRes &&
        tvRes.data &&
        tvRes.data.Component_Play_Playinfo &&
        tvRes.data.Component_Play_Playinfo.urls &&
        Object.keys(tvRes.data.Component_Play_Playinfo.urls).length > 0
      ) {
    ...
    const setName = getName(
      GM_getValue('retweetMode', false) && retweetPostId
      ...
      postId,
      postUid,
      index.toString().padStart(padLength, '0'),
      postTime,
      text,
      retweetPostId,
      retweetUserName,
  ...
  function handlePic(
    ...
    let downloadUrl = GM_getValue('rmWtrMrk', false)
      ? largePicUrl
      : 'https://weibo.com/ajax/common/download?pid=' + picId;
    let picName = largePicUrl.split('/')[largePicUrl.split('/').length - 1].split('?')[0];
    let originalName = picName.split('.')[0];
    let ext = picName.split('.')[1];
    const setName = getName(
  ...
  function addDlBtn(footer) {
    ...
    dlBtn.innerHTML =
      '<span class="woo-like-iconWrap"><i class="woo-font woo-font--imgSave woo-like-icon"></i></span><span class="woo-like-count">下载</span>';
    dlBtn.addEventListener('click', async function (event) {
      ...
      if (article) {
        ...
        const packName = getName(
          GM_getValue('retweetMode', false) && retweetPostId
            ? GM_getValue('retweetPackFileName', '{mblogid}.zip')
            : GM_getValue('packFileName', '{mblogid}.zip'),
          '{original}',
          '{ext}',
          userName,
          userId,
  ...
  function addSingleDlBtn(img, idx = 0) {
    ...
    dlBtn.addEventListener('mouseenter', (event) => {
      dlBtn.style.backgroundColor = 'rgba(255, 255, 255, 0.8)';
      dlBtn.style.color = 'black';
    });
    dlBtn.addEventListener('mouseleave', (event) => {
      dlBtn.style.backgroundColor = 'rgba(255, 255, 255, 0.4)';
      dlBtn.style.color = 'dimgray';
    });
    dlBtn.addEventListener('click', async function (event) {
  ...
  function sAddDlBtn(footer) {
    ...
    dlBtn.innerHTML =
      '<span class="woo-like-iconWrap"><svg class="woo-like-icon"><use xlink:href="#woo_svg_download"></use></svg></span><span class="woo-like-count">下载</span>';
...
*/
  function handleCard(card) {
    ...
    const imgs = card.querySelectorAll(
      'img.woo-picture-img,img.picture_focusImg_1z5In,img.picture-viewer_pic_37YQ3,video.picture-viewer_pic_37YQ3'
    );
    // console.log(imgs);
    if (footer) {
      if (footer.getElementsByClassName('download-button').length > 0) {
...
*/
  ...
  function showModal(event) {
    ...
    inputPackName.style.borderColor =
      GM_getValue('zipMode', false) && !GM_getValue('ariaMode', false) ? 'black' : 'gray';
    ...
    labelRetweetPackName.style.color =
      GM_getValue('zipMode', false) && GM_getValue('retweetMode', false) && !GM_getValue('ariaMode', false)
        ? 'black'
        : 'gray';
    ...
    okButton.addEventListener('click', function (event) {
      ...
      if (
        ...
        !document.getElementById('dlFileName').value.includes('{original}') &&
        !document.getElementById('dlFileName').value.includes('{index}')
      ) {
  ...
  symbol.innerHTML =
    '<path d="m25,0l50,0l0,50l25,0l-50,50l-50,-50l25,0l0,-50" fill="currentColor"></path><path d="m30,5l40,0l0,50l20,0l-40,40l-40,-40l20,0l0,-50" fill="white"></path>';
    ...
    if (location.host == 'weibo.com' || location.host == 'www.weibo.com') {
      ...
      for (const mutation of mutationList) {
        ...
        if (
          ...
          (mutation.target.className.includes('wbpro-feed-content') ||
            mutation.target.className.includes('Feed_retweet_JqZJb'))
        ) {
